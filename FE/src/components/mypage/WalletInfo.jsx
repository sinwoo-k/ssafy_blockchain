...
const WalletInfo = () => {
  ...
  if (walletInfo.isLoading) {
    return (
    <div className="fixed top-[70px] right-4 z-40 bg-gray-800/90 backdrop-blur-sm rounded-lg p-2.5 shadow-lg border border-gray-700 w-[190px]">
        <div className="flex items-center justify-between">
          ...
          <div className="animate-spin rounded-full h-3 w-3 border-t-2 border-b-2 border-[#3cc3ec]"></div>
        </div>
      </div>
    );
  }
  ...
  if (walletInfo.error) {
    ...
    <div className="fixed top-[70px] right-4 z-40 bg-gray-800/90 backdrop-blur-sm rounded-lg p-2.5 shadow-lg border border-gray-700 w-[190px]">
        ...
        <div className="text-xs text-red-500 mt-1">{walletInfo.error}</div>
      </div>
    );
  }

  return (
    <div className="fixed top-[150px] right-50 z-40 bg-gray-800/90 backdrop-blur-sm rounded-lg p-2.5 shadow-lg border border-gray-700 w-[190px]">
      <div className="flex items-center justify-between mb-2">
        ...
        {walletInfo.walletAddress && (
          <div className="relative group">
            <div 
              className="text-xs font-mono bg-gray-700 px-1.5 py-0.5 rounded cursor-pointer flex items-center"
            ...
            {/* 전체 지갑 주소 툴팁 */}
            <div className="absolute left-0 -bottom-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50">
              <div className="bg-black text-white text-[10px] p-1 rounded whitespace-nowrap overflow-hidden max-w-[220px]">
                {walletInfo.walletAddress}
              </div>
            </div>
            ...
            {/* 복사 성공 메시지 */}
            {copied && (
              <div className="absolute right-0 -bottom-6 bg-green-900 text-white text-[10px] py-0.5 px-1 rounded z-50">
                복사됨!
              </div>
            )}
          </div>
      ...
      {/* ETH 잔액 */}
      <div className="bg-gray-700 p-2 rounded mb-1.5">
        <div className="text-[10px] text-gray-400">ETH 잔액</div>
        <div className="text-sm font-medium">{walletInfo.amount} ETH</div>
      </div>
      
      {/* 통화 변환 섹션 */}
      <div className="bg-gray-700 p-2 rounded relative">
        <div className="flex items-center justify-between">
          <div className="text-[10px] text-gray-400 flex items-center gap-1">
            {walletInfo.currency} 
            <button onClick={toggleCurrency} className="text-blue-400 hover:text-blue-300">
          ...
          >
            {refreshing ? (
              <span className="animate-spin inline-block h-3 w-3 border-t border-b border-white rounded-full" />
            ) : (
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="text-sm font-medium w-full" style={{ wordBreak: 'break-all' }}>
