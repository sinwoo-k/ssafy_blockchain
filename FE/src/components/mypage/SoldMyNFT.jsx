...
const SoldMyNFT = () => {
  ...
  useEffect(() => {
    // API 호출하여 판매 완료한 NFT 목록 데이터 가져오기
    fetchSoldData();
  }, []);
  ...
  const fetchSoldData = async () => {
    try {
      setLoading(true);
      const response = await userService.selledMyNFT();
      ...
      console.error('판매 완료한 NFT 목록 로드 오류:', err);
      setError('판매 완료한 NFT 목록을 불러오는 중 오류가 발생했습니다.');
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="mb-2 grid grid-cols-5 text-xs text-gray-400 border-b border-gray-800 pb-2">
        ...
        <div>작품명</div>
        <div className="text-center">판매 가격</div>
        <div className="text-center">구매자</div>
          ...
          auctions.map((item) => (
            <div key={item.auctionItemId || item.nftId} className="grid grid-cols-5 items-center py-3 border-b border-gray-800">
              ...
              <div className="text-sm text-center truncate">
                {item.buyerName || item.buyer || '구매자'}
              </div>
              <div className="text-sm text-center">
                {formatDate(item.soldDate || item.endTime || item.startTime)}
              </div>
            </div>
          ))
        ) : (
